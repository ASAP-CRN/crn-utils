...
def get_field_transfer_map() -> pd.DataFrame:
    ...
    return pd.DataFrame(
        transfers, columns=["Old_Table", "New_Table", "Old_Field", "New_Field"]
    )


def apply_field_moves(
    tables: dict[str, pd.DataFrame], transfer_map: pd.DataFrame
    ...
    for _, row in transfer_map.iterrows():
        ...
        if old_field not in src.columns:
            print(
                f"WARNING: Field '{old_field}' not found in '{old_table}', skipping transfer"
            )
            continue
...
def update_table_columns(
    ...
    if deprecated_cols:
        print(
            f"Deprecated columns for {table_name} will be removed: {', '.join(sorted(deprecated_cols))}"
        )
    ...
    if cols_to_add:
        print(
            f"New columns for {table_name} will be added: {', '.join(sorted(cols_to_add))}"
        )
    ...
    if extra_cols:
        print(
            f"Extra columns for {table_name} will be kept as-is: {', '.join(sorted(extra_cols))}"
        )
...
def update_metadata_to_version(
    ...
    for table_name in expected_tables:
        ...
        if table_name not in current_tables:
            print(
                f"WARNING: {table_name} not found in old version, creating empty table"
            )
            new_cols = new_cde[new_cde["Table"] == table_name]["Field"].tolist()
    ...
    if extra_tables:
        print(
            f"WARNING: Removing tables that are not expected for '{source}': {', '.join(sorted(extra_tables))}"
        )
...
def v1_to_v2(
    ...
    if out_dir is not None:
        ...
        for table_name, aux_df in aux_tables.items():
            aux_df.to_csv(
                os.path.join(export_root, f"{table_name}_aux.csv"), index=False
            )
