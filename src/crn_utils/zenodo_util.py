...
def setup_zenodo():
    ...
    if not api_token:
        raise ValueError(
            "ZENODO_API_TOKEN not found in environment variables. Please set it in your .env file."
        )
...
def create_deposition(api_token):
    ...
    if response.status_code != 201:
        raise Exception(
            f"Error creating deposition: {response.status_code}\n{response.text}"
        )
...
def upload_file(deposition, file_path, api_token):
    ...
    if response.status_code not in (200, 201):
        raise Exception(
            f"Error uploading file: {response.status_code}\n{response.text}"
        )
...
def update_metadata(deposition, api_token, metadata_payload):
    ...
    response = requests.put(
        url, params=params, data=json.dumps(data_payload), headers=headers
    )
    if response.status_code != 200:
        raise Exception(
            f"Error updating metadata: {response.status_code}\n{response.text}"
        )
...
def publish_deposition(deposition, api_token):
    ...
    if response.status_code != 202:
        raise Exception(
            f"Error publishing deposition: {response.status_code}\n{response.text}"
        )
...
def instructions():
    ...
    print(
        "(3) Place a Zenodo API key with deposit:actions & deposite:write access in a .env file."
    )
    print("(4) Install dependencies 'pip install python-dotenv'.")
    print(
        "(5) Comment out this function and uncomment main, run it and check your Zenodo account for published DOIs."
    )
    print(
        "** Please walk through the code yourself, run a test trial on a single upload before running all, and feel ask any questions. **"
    )
