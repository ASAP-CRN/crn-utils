...
def validate_table(
    ...
    for field in specific_cde_df["Field"]:
        ...
        opt_req = (
            "REQUIRED"
            if specific_cde_df.loc[entry_idx, "Required"].item() == "Required"
            else "OPTIONAL"
        )
    ...
    if len(missing_required) > 0:
        out.add_error(
            f"Missing Required Fields in {table_name}: {', '.join(missing_required)}"
        )
    ...
    if len(missing_optional) > 0:
        out.add_error(
            f"Missing Optional Fields in {table_name}: {', '.join(missing_optional)}"
        )
...
def calculate_missingness(
    ...
    required_fields = (
        schema[schema["Required"] == "Required"]["Field"].str.lower().to_list()
    )
    optional_fields = (
        schema[schema["Required"] == "Optional"]["Field"].str.lower().to_list()
    )
    ...
    required_missingness_by_ds = df.groupby("dataset")[required_fields].apply(
        missingness
    )
    optional_missingness_by_ds = df.groupby("dataset")[optional_fields].apply(
        missingness
    )

    # apply threshold to get the "missing" fields
    required_missing = required_missingness[
        required_missingness > missing_threshold
    ].index.to_list()
    optional_missing = optional_missingness[
        optional_missingness > missing_threshold
    ].index.to_list()

    missing_required_by_ds = (
        (required_missingness_by_ds > missing_threshold)
        .apply(get_true_columns, axis=1)
        .to_dict()
    )
    missing_optional_by_ds = (
        (optional_missingness_by_ds > missing_threshold)
        .apply(get_true_columns, axis=1)
        .to_dict()
    )
    ...
    required_missingness_by_ds = required_missingness_by_ds.map(
        lambda x: f"{100*x:.1f}%"
    )
    optional_missingness_by_ds = optional_missingness_by_ds.map(
        lambda x: f"{100*x:.1f}%"
    )
...
"""
    tmp = (
        required_missingness.reset_index()
        .rename(columns={"index": "Required Fields", 0: "Missingness"})
        .to_markdown(index=False)
    )
    report += f"{tmp}\n"

    tmp = (
        optional_missingness.reset_index()
        .rename(columns={"index": "Optional Fields", 0: "Missingness"})
        .to_markdown(index=False)
    )
    report += f"\n\n{tmp}\n"
